...
private:
  ...
  const ulong TIMEOUT_CANCEL = 50000; //in ms; Cancel operation after ... ms
  boolean timeout_active = false;
  ulong timeout_start;

  // RETRY_INTERVAL changed from 5000 to 30000 on 14032023
  const ulong RETRY_INTERVAL = 30000; //5000; //in ms; first retry after ... ms; second retry after 2 * ... ms; third after 4 ...
  ulong retry_start = 0;

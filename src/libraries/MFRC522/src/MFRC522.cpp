#line 1 "D:\\AmplifyMobility\\Test COde\\Code- AC001-Shobha demo\\AC001_Three_Connectors\\src\\libraries\\MFRC522\\src\\MFRC522.cpp"
/*
...
void MFRC522::PCD_WriteRegister(	PCD_Register reg,	///< The register to write to. One of the PCD_Register enums.
	...
	hspiRfid->beginTransaction(SPISettings(MFRC522_SPICLOCK, MSBFIRST, SPI_MODE0));	// Set the settings to work with SPI bus
	digitalWrite(_chipSelectPin, LOW);		// Select slave
	hspiRfid->transfer(reg);						// MSB == 0 is for writing. LSB is not used in address. Datasheet section 8.1.2.3.
	hspiRfid->transfer(value);
	digitalWrite(_chipSelectPin, HIGH);		// Release slave again
	hspiRfid->endTransaction(); // Stop using the SPI bus
...
void MFRC522::PCD_WriteRegister(	PCD_Register reg,	///< The register to write to. One of the PCD_Register enums.
	...
	for (byte index = 0; index < count; index++) {
		hspiRfid->transfer(values[index]);
	}
	digitalWrite(_chipSelectPin, HIGH);		// Release slave again
	hspiRfid->endTransaction(); // Stop using the SPI bus
} // End PCD_WriteRegister()
...
byte MFRC522::PCD_ReadRegister(	PCD_Register reg	///< The register to read from. One of the PCD_Register enums.
	...
	hspiRfid->beginTransaction(SPISettings(MFRC522_SPICLOCK, MSBFIRST, SPI_MODE0));	// Set the settings to work with SPI bus
	digitalWrite(_chipSelectPin, LOW);			// Select slave
	hspiRfid->transfer(0x80 | reg);					// MSB == 1 is for reading. LSB is not used in address. Datasheet section 8.1.2.3.
	value = hspiRfid->transfer(0);					// Read the value back. Send 0 to stop reading.
	digitalWrite(_chipSelectPin, HIGH);			// Release slave again
	hspiRfid->endTransaction(); // Stop using the SPI bus
	return value;
...
void MFRC522::PCD_ReadRegister(	PCD_Register reg,	///< The register to read from. One of the PCD_Register enums.
	...
	hspiRfid->transfer(address);					// Tell MFRC522 which address we want to read
	if (rxAlign) {		// Only update bit positions rxAlign..7 in values[0]
		// Create bit mask for bit positions rxAlign..7
		byte mask = (0xFF << rxAlign) & 0xFF;
		// Read value and tell that we want to read the same address again.
		byte value = hspiRfid->transfer(address);
		// Apply mask to both current value of values[0] and the new data in value.
		values[0] = (values[0] & ~mask) | (value & mask);
		index++;
	}
	while (index < count) {
		values[index] = hspiRfid->transfer(address);	// Read value and tell that we want to read the same address again.
		index++;
	}
	values[index] = hspiRfid->transfer(0);			// Read the final byte. Send 0 to stop reading.
	digitalWrite(_chipSelectPin, HIGH);			// Release slave again
	hspiRfid->endTransaction(); // Stop using the SPI bus
} // End PCD_ReadRegister()

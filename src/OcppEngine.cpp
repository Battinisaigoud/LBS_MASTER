...
ChargePointStatusService *ocppEngine_chargePointStatusService_A;
ChargePointStatusService *ocppEngine_chargePointStatusService_B;
//ChargePointStatusService *ocppEngine_chargePointStatusService_C;
...
boolean processWebSocketEvent(uint8_t * payload, size_t length){
  ...
  switch (err.code()) {
    ...
    case DeserializationError::InvalidInput:
        Serial.print("[OcppEngine] Invalid input! Not a JSON\n");
        break;
    case DeserializationError::NoMemory:
        Serial.print("[OcppEngine] Error: Not enough memory\n");
        break;
    default:
        Serial.print("[OcppEngine] Deserialization failed\n");
        break;
  }
  ...
  switch(messageTypeId) {
    ...
    case MESSAGE_TYPE_CALLERROR:
      ...
      break;
    default:
      Serial.print("[OcppEngine] Invalid OCPP message! (though JSON has successfully been deserialized)\n");
      break;
  }
...
void handleReqMessage(JsonDocument *json){
  ...
  if (req == NULL) {
    Serial.print("[OcppEngine] Couldn't make OppOperation from Request. Ignore request.\n");
    return;
  }
...
void handleConfMessage(JsonDocument *json){
  ...
  Serial.print("[OcppEngine] Received CALLRESULT doesn't match any pending operation!\n");
}
...
void ocppEngine_loop(){
   ...
   */
  Serial.println("[ocppEngine_loop]");
  while (initiatedOcppOperations.size() > 0){
    if (gu8_online_flag == 1)
    {
      ...
      if (timeout)
      { // the Conf msg processing routine dequeues finished elements
...
SmartChargingService* getSmartChargingService(){
  if (ocppEngine_smartChargingService == NULL) {
    Serial.print("[OcppEngine] Error: in OcppEngine, there is no smartChargingService set, but it is accessed!\n");
    //no error catch 
  }
...
void setChargePointStatusService(ChargePointStatusService *cpss){
  if(i == 0){
    ocppEngine_chargePointStatusService_A = cpss;
    i++;
    Serial.println("CCS A is Set");
  }else if(i == 1){
    ocppEngine_chargePointStatusService_B = cpss;
    i++;
    Serial.println("CCS B is Set");
  }


}

ChargePointStatusService *getChargePointStatusService_A(){
  if (ocppEngine_chargePointStatusService_A == NULL) {
    Serial.print("[OcppEngine_A] Error: in OcppEngine, there is no chargePointStatusService set, but it is accessed!\n");
    //no error catch 
  }
  return ocppEngine_chargePointStatusService_A;
}

ChargePointStatusService *getChargePointStatusService_B(){
  if (ocppEngine_chargePointStatusService_B == NULL) {
    Serial.print("[OcppEngine_B] Error: in OcppEngine, there is no chargePointStatusService set, but it is accessed!\n");
    //no error catch 
  }
  return ocppEngine_chargePointStatusService_B;
}

// ChargePointStatusService *getChargePointStatusService_C(){
//   if (ocppEngine_chargePointStatusService_C == NULL) {
//     Serial.print(F("[OcppEngine_C] Error: in OcppEngine, there is no chargePointStatusService set, but it is accessed!\n"));
//     //no error catch 
//   }
//   return ocppEngine_chargePointStatusService_C;
// }
...
MeteringService* getMeteringService() {
  if (ocppEngine_meteringService == NULL) {
    Serial.print("[OcppEngine] Error: in OcppEngine, there is no ocppEngine_meteringService set, but it is accessed!\n");
    //no error catch 
  }

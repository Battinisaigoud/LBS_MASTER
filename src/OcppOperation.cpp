...
char Boot_Notification[] = "BootNotification";
uint8_t Boot_Notification_Result = 0xFF;
...
void OcppOperation::setMessageID(String &id){
  if (messageID.length() > 0){
    Serial.print("[OcppOperation] OcppOperation (class): MessageID is set twice or is set after first usage!\n");
...
boolean OcppOperation::sendReq(){
  ...
  if (timeout_active && millis() - timeout_start >= TIMEOUT_CANCEL){
    ...
    Serial.print("[OcppOperation] ");
    Serial.print(ocppMessage->getOcppOperationType());
    Serial.print(" timeout! Cancel operation\n");
    char Boot_Notification_Cmp[100];
    strcpy(Boot_Notification_Cmp,ocppMessage->getOcppOperationType());
    Serial.println("Boot_Notification_Cmp"+String(Boot_Notification_Cmp));
    Boot_Notification_Result = strcmp(Boot_Notification, Boot_Notification_Cmp);
    if(Boot_Notification_Result == 0)
    {
  ...
  if (DEBUG_OUT) Serial.print("[OcppOperation] Send requirement: ");
  if (DEBUG_OUT) Serial.print(out);
  if (DEBUG_OUT) Serial.print("\n");

if(wifi_connect == true || ethernet_connect == true){
  Serial.println("[Sending data via Wifi/ETH]");
//#if WIFI_ENABLED || ETHERNET_ENABLED
  if (webSocket->sendTXT(out)) {
    //success
   // Serial.println("Why me"); //@wamique
    if (!timeout_active) {
      timeout_active = true;
...
//#if GSM_ENABLED
  ...
  //    Serial.println("Why me");
    ...
    //webSocket is not able to put any data on TCP stack. Maybe because we're offline
    Serial.println("webSocket is not able to put any data on TCP stack. Maybe because we're offline");
    retry_start = 0;
    retry_interval_mult = 1;
  }
  client.flush();
...
//#if GSM_ENABLED
 success = sendTXTGsmStr(out);
 Serial.println("[GSM: ]SendConf" +String(out)+String(success));
 client.flush();
//#endif
}

  if (success) {
    if (DEBUG_OUT) Serial.print("[OcppOperation] (Conf) JSON message: ");
    if (DEBUG_OUT) Serial.print(out);
    if (DEBUG_OUT) Serial.print('\n');
...
void OcppOperation::print_debug() {
  Serial.print("[OcppOperation] I am a ");
  if (ocppMessage) {
    Serial.print(ocppMessage->getOcppOperationType());
  } else {
    Serial.print("NULL");

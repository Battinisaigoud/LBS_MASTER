...
extern uint8_t gu8_gsm_setup_counter;
...
void gsm_Loop()
{
  ...
  if (!client.connected())
  {
    ...
    {
      // if(getChargePointStatusService_A()->getEmergencyRelayClose() == false && getChargePointStatusService_B()->getEmergencyRelayClose() == false){
    ...
    if (gu8_gsm_setup_counter >= 10)
    {
      Serial.print("gu8_gsm_setup_counter");
      Serial.println(gu8_gsm_setup_counter);
      gu8_gsm_setup_counter = 0;
      SetupGsm();
    }
...
void gsmOnEvent()
{
  ...
  while (client.connected() && client.available())
  {
    ...
    { // End of Headers
      ...
      {
        CounterOpenBracket++;
        Serial.println("CounterOpenBracket" + String(CounterOpenBracket));
        flagCounter = 1;
      }
      if (c == ']')
      {
        CounterCloseBracket++;
        Serial.println("CounterCloseBracket" + String(CounterCloseBracket));
      }

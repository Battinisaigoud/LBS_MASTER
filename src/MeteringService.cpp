...
void MeteringService::addDataPoint(time_t currentTime, float currentPower, float currentEnergy, float currentVoltage, float currentCurrent, float currentTemperature, int connector) {
	...
	if (getChargePointStatusService_A() != NULL && getChargePointStatusService_A()->getTransactionId() == -1)
	{
	...
	else if (connector == 1 && getChargePointStatusService_A() != NULL && getChargePointStatusService_A()->getConnectorId() == 1) {
		sampleTimeA.add(currentTime);
	...
	if (connector == 1) {
		...
		if (gu8_online_flag == 1)
		{
			OcppOperation* meterValues = makeOcppOperation(webSocket,
				...
				new MeterValues(&sampleTimeA, &powerA, &energyA, &voltageA, &currentA, &temperature)); //have to pass connector
...
void MeteringService::loop() {
	...
	if (DEBUG_OUT_M) Serial.println(lastSampleTimeA + meterSampleInterval);
	if (DEBUG_OUT_M) Serial.println("The now() is: ");
	if (DEBUG_OUT_M) Serial.println(now());

	if ((now() >= (time_t)meterSampleInterval + lastSampleTimeA)) {
		if (energyASampler != NULL
			...
			float sampledEnergy_A = setPresentEnergyASampler(sampledVoltage_A, sampledCurrent_A, deltaA);
			float sampledTemperature = temperatureSampler();

			if (getChargePointStatusService_A()->getUnavailabilityStatus() == false) {
				connectorNo = 1;
			}/*else if(getChargePointStatusService_B()->getUnavailabilityStatus() == false){
				connectorNo  = 2;
			}*/else {
				Serial.println("ChargePoint_Status_A: ");
				Serial.println(getChargePointStatusService_A()->getUnavailabilityStatus());
				/*Serial.println("ChargePoint_Status_B: ");
				Serial.println(getChargePointStatusService_B()->getUnavailabilityStatus());
				Serial.println("[MeteringService] SomeThing is wrong with Connector Id");*/
				//delay(5000);
			}

			addDataPoint(sampledTimeA, sampledPower_A, sampledEnergy_A, sampledVoltage_A, sampledCurrent_A, sampledTemperature, connectorNo); //1 = conector ID
...
void MeteringService::init(MeteringService* meteringService) {
	...
	*/
	...
	meteringService->setEnergyASampler([](float volt_A, float current_A, time_t delta_A) {
		...
		Serial.println("[EnergyASampler] delta_A: " + String(delta_A));
...
float MeteringService::currentEnergy_A()
{
	...
	// float finalEnergy_A = lastEnergy_A;
...
float MeteringService::setPresentEnergyASampler(float volt, float current, time_t delta)
{
	...
	Serial.println(" currEnergy_A: " + String(current_energy_A));
	Serial.println("LastPresentEnergy_A: " + String(LastPresentEnergy_A));
	Serial.println("discurrEnergy_A: " + String(discurrEnergy_A));
	Serial.println("[EnergyASampler] finalEnergy_A: " + String(finalEnergy_A));
	lbs_metervalues = discurrEnergy_A;
